...
//            const auto id_j = (*fInRecHits.ID)[rh_j];
    ...
    else if (fSkim == SkimType::DiXtal) // this is a hack selection, which mixes up seeds and photons --> do NOT use this for analysis
    {
      ...
      std::cout << "GetEntry for fInRecHits E,ID,time done "<< std::endl;

      gZmass = 0.f;     
      gdR = 0.f;
      std::cout << "DiXtal loop over rechits"<< std::endl;
      ...
      std::cout << "DiXtal loop over photons "<< std::endl;
	...
	// get pair of rechits that are good candidates : double loop, yo
        std::cout << "DiXtal getting rec hits "<< std::endl;
	inpho.b_recHits->GetEntry(entry);

	const auto n = inpho.recHits->size();
        std::cout << "DiXtal loop over rechits with " << n << std::endl;
	for (auto i = 0U; i < n; i++)
	{
	  Bool_t isGoodPair = false;

          std::cout << "-- Getting rechit infor from fInRechits with "<< i << std::endl;
	  const auto rh_i = (*inpho.recHits)[i]; // position within event rec hits vector
          std::cout << "-- Getting rechit infor from fInRechits rh_i " << rh_i << std::endl;
          std::cout << "-- Getting rechit infor from fInRechits #  " << (fInRecHits.E)->size() << std::endl;
	  const auto E_i  = (*fInRecHits.E) [rh_i];
          std::cout << "-- Getting rechit infor from fInRechits E_i"<< std::endl;
	  const auto id_i = (*fInRecHits.ID)[rh_i];
          std::cout << "-- Getting rechit infor from fInRechits id_i"<< std::endl;
          const auto t_i = (*fInRecHits.time)[rh_i];
          std::cout << "-- Getting rechit infor from fInRechits t_i "<< std::endl;
...
{  
  ...
  for (auto ipho : fPhoList)
  {
    ...
    if (fInConfig.storeRecHits)
    {
      ...
      inpho.b_recHits->GetEntry(entry);
      inpho.b_kurecHits->GetEntry(entry);
    }
    else
    {
  ...
  if (fInConfig.storeRecHits)
  {
    ...
    // fInKURecHits.b_X->GetEntry(entry);
    // fInKURecHits.b_Y->GetEntry(entry);
    // fInKURecHits.b_Z->GetEntry(entry);
    fInKURecHits.b_E->GetEntry(entry);
    fInKURecHits.b_time->GetEntry(entry);
    // fInKURecHits.b_timeErr->GetEntry(entry);
    fInKURecHits.b_TOF->GetEntry(entry);
    fInKURecHits.b_ID->GetEntry(entry);
    // fInKURecHits.b_isOOT->GetEntry(entry);
    fInKURecHits.b_isGS6->GetEntry(entry);
    fInKURecHits.b_isGS1->GetEntry(entry);
    fInKURecHits.b_adcToGeV->GetEntry(entry);
    fInKURecHits.b_ped12->GetEntry(entry);
    fInKURecHits.b_ped6->GetEntry(entry);
    fInKURecHits.b_ped1->GetEntry(entry);
    fInKURecHits.b_pedrms12->GetEntry(entry);
    fInKURecHits.b_pedrms6->GetEntry(entry);
    fInKURecHits.b_pedrms1->GetEntry(entry);
...
void Skimmer::InitInBranchVecs()
{
  ...
  if (fInConfig.storeRecHits) 
  {
    ...
    fInKURecHits.E = 0; 
    fInKURecHits.time = 0;
    // fInKURecHits.timeErr = 0;
    fInKURecHits.TOF = 0;
    fInKURecHits.ID = 0;
    // fInKURecHits.isOOT = 0;
    fInKURecHits.isGS6 = 0;
    fInKURecHits.isGS1 = 0; 
    fInKURecHits.adcToGeV = 0;
    fInKURecHits.ped12 = 0;
    fInKURecHits.ped6 = 0;
    fInKURecHits.ped1 = 0;
    fInKURecHits.pedrms12 = 0;
    fInKURecHits.pedrms6 = 0;
    fInKURecHits.pedrms1 = 0;

    for (auto ipho = 0; ipho < Common::nPhotons; ipho++) 
    {
      fInPhos[ipho].recHits = 0;
      fInPhos[ipho].kurecHits = 0;
    }  
  }
}
...
void Skimmer::InitInBranches()
{
  ...
  if (fInConfig.storeRecHits)
  {
    ...
    // fInTree->SetBranchAddress(fInKURecHits.s_X.c_str(), &fInKURecHits.X, &fInKURecHits.b_X);
    // fInTree->SetBranchAddress(fInKURecHits.s_Y.c_str(), &fInKURecHits.Y, &fInKURecHits.b_Y);
    // fInTree->SetBranchAddress(fInKURecHits.s_Z.c_str(), &fInKURecHits.Z, &fInKURecHits.b_Z);
    fInTree->SetBranchAddress(fInKURecHits.s_E.c_str(), &fInKURecHits.E, &fInKURecHits.b_E);
    fInTree->SetBranchAddress(fInKURecHits.s_time.c_str(), &fInKURecHits.time, &fInKURecHits.b_time);
    // fInTree->SetBranchAddress(fInKURecHits.s_timeErr.c_str(), &fInKURecHits.timeErr, &fInKURecHits.b_timeErr);
    fInTree->SetBranchAddress(fInKURecHits.s_TOF.c_str(), &fInKURecHits.TOF, &fInKURecHits.b_TOF);
    fInTree->SetBranchAddress(fInKURecHits.s_ID.c_str(), &fInKURecHits.ID, &fInKURecHits.b_ID);
    // fInTree->SetBranchAddress(fInKURecHits.s_isOOT.c_str(), &fInKURecHits.isOOT, &fInKURecHits.b_isOOT);
    fInTree->SetBranchAddress(fInKURecHits.s_isGS6.c_str(), &fInKURecHits.isGS6, &fInKURecHits.b_isGS6);
    fInTree->SetBranchAddress(fInKURecHits.s_isGS1.c_str(), &fInKURecHits.isGS1, &fInKURecHits.b_isGS1);
    fInTree->SetBranchAddress(fInKURecHits.s_adcToGeV.c_str(), &fInKURecHits.adcToGeV, &fInKURecHits.b_adcToGeV);
    fInTree->SetBranchAddress(fInKURecHits.s_ped12.c_str(), &fInKURecHits.ped12, &fInKURecHits.b_ped12);
    fInTree->SetBranchAddress(fInKURecHits.s_ped6.c_str(), &fInKURecHits.ped6, &fInKURecHits.b_ped6);
    fInTree->SetBranchAddress(fInKURecHits.s_ped1.c_str(), &fInKURecHits.ped1, &fInKURecHits.b_ped1);
    fInTree->SetBranchAddress(fInKURecHits.s_pedrms12.c_str(), &fInKURecHits.pedrms12, &fInKURecHits.b_pedrms12);
    fInTree->SetBranchAddress(fInKURecHits.s_pedrms6.c_str(), &fInKURecHits.pedrms6, &fInKURecHits.b_pedrms6);
    fInTree->SetBranchAddress(fInKURecHits.s_pedrms1.c_str(), &fInKURecHits.pedrms1, &fInKURecHits.b_pedrms1);
  }
  ...
  for (auto ipho = 0; ipho < Common::nPhotons; ipho++) 
  {
    ...
    {
      // fInTree->SeaBranchAddress(Form("%s_%i",pho.s_seedX.c_str(),ipho), &pho.seedX, &pho.b_seedX);
...
void Skimmer::InitOutBranches()
{
  ...
  for (auto ipho = 0; ipho < fNOutPhos; ipho++) 
  {
    ...
    // fOutTree->Branch(Form("%s_%i",pho.s_seedkuX.c_str(),ipho), &pho.seedkuX);
    // fOutTree->Branch(Form("%s_%i",pho.s_seedkuY.c_str(),ipho), &pho.seedkuY);
    // fOutTree->Branch(Form("%s_%i",pho.s_seedkuZ.c_str(),ipho), &pho.seedkuZ);
    ...
    // fOutTree->Branch(Form("%s_%i",pho.s_seedkuisOOT.c_str(),ipho), &pho.seedkuisOOT);
    fOutTree->Branch(Form("%s_%i",pho.s_seedkuisGS6.c_str(),ipho), &pho.seedkuisGS6);
    fOutTree->Branch(Form("%s_%i",pho.s_seedkuisGS1.c_str(),ipho), &pho.seedkuisGS1);
    fOutTree->Branch(Form("%s_%i",pho.s_seedkuadcToGeV.c_str(),ipho), &pho.seedkuadcToGeV);
    fOutTree->Branch(Form("%s_%i",pho.s_seedkuped12.c_str(),ipho), &pho.seedkuped12);
    fOutTree->Branch(Form("%s_%i",pho.s_seedkuped6.c_str(),ipho), &pho.seedkuped6);
    fOutTree->Branch(Form("%s_%i",pho.s_seedkuped1.c_str(),ipho), &pho.seedkuped1);
    fOutTree->Branch(Form("%s_%i",pho.s_seedkupedrms12.c_str(),ipho), &pho.seedkupedrms12);
    fOutTree->Branch(Form("%s_%i",pho.s_seedkupedrms6.c_str(),ipho), &pho.seedkupedrms6);
    fOutTree->Branch(Form("%s_%i",pho.s_seedkupedrms1.c_str(),ipho), &pho.seedkupedrms1);
    ...
    if (fInConfig.storeRecHits)
    {
      ...
      fOutTree->Branch(Form("%s_%i",pho.s_nkurechits.c_str(),ipho), &pho.nkurechits);
