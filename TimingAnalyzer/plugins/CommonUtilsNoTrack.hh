...
namespace ConfigNT
{
  ...
  inline float hypo  (const float x, const float y, const float z = 0.f){return std::sqrt(ConfigNT::rad2(x,y,z));}
  ...
  {
    return -1.0f*std::log(std::tan(ConfigNT::theta(ConfigNT::hypo(x,y),z)/2.f));
  }
...
namespace ootNT
{
  ...
  inline float GetPhotonEt(const reco::Photon & photon){return (photon.et());}//*(photon.userFloat("ecalEnergyPostCorr")/photon.energy()));}
  inline float GetPhotonPt(const reco::Photon & photon){return (photon.pt());}//*(photon.userFloat("ecalEnergyPostCorr")/photon.energy()));}

  // sort by photon corrected pt
  inline void SortPhotonsByPt(std::vector<reco::Photon> & photons)
  {
    std::sort(photons.begin(),photons.end(),
	      [](const auto & photon1, const auto & photon2)
	      {
		return ootNT::GetPhotonPt(photon1) > ootNT::GetPhotonPt(photon2);
	      });
  }
 ...
 void PrepRecHits(const EcalRecHitCollection * recHitsEB, 
  ...
  void MergePhotons(const edm::Handle<std::vector<reco::Photon> > & photonsH, const int nPho, const std::vector<int> & pho_overlap_idxs,
		    std::vector<reco::Photon> & photons, const bool isOOT, const float rho, const float phpTmin = 0.f, const std::string & phIDmin = "none");
  ...
  void GetGEDPhoVIDByHand(const reco::Photon & photon, std::vector<reco::Photon::IdPair> & idpairs);
  ...
  void PrepLeptons(const edm::Handle<std::vector<Lep> > & lepsH, std::vector<Lep> & leps,
		   const std::vector<reco::Photon> & photons, const float leppTmin = 0.f, 
			   ...
			   strBitMap & isHLTMatched, const Obj & obj, const float pTres = 1.f, const float dRmin = 100.f)
  {
    for (const auto & triggerObjectsByFilterPair : triggerObjectsByFilterMap)
    {
      ...
      const bool isL1T = (filterName == ConfigNT::L1Trigger.c_str());
