...
void wc_ku_InterCali_v1( string infilename ){
    ...
    float nM[nIterations] = {0.f};
    const int power = 2;

    auto MHist = new TH1F( "MHistV3", "M Histogram V3", nIterations, 0, nIterations);
    auto MnotHist = new TH1F( "MnotHistV3", "Mnot Histogram V3", nIterations, 0, nIterations);
...
//{sumXtalRtStcPhoIcRecTime, sumXtalRtOOTStcPhoIcRecTime }; //, sumXtalWtOOTStcPhoIcRecTime };
    ...
    for( auto i = 0; i < nAlgos; i++){
		...
		string hnameEB( "CaliV3"+algostring[i]+"PhoIcRecTimeEBMap_i"+to_string(j)); 
                string htitleEB( "CaliV3"+algostring[i]+"IcRecTimeEBMap EB "+to_string(j));
                IcMapEB[i][j] = new TH2F(hnameEB.c_str(),htitleEB.c_str(),171,-85.5,85.5,360,0.5,360.5);
                IcMapEB[i][j]->Sumw2();
                string hnameEP( "CaliV3"+algostring[i]+"PhoIcRecTimeEPMap_i"+to_string(j)); 
                string htitleEP( "CaliV3"+algostring[i]+"IcRecTimeEPMap EB "+to_string(j));
                IcMapEP[i][j] = new TH2F(hnameEP.c_str(),htitleEP.c_str(),100,0.5,100.5,100,0.5,100.5);
                IcMapEP[i][j]->Sumw2();
                string hnameEM( "CaliV3"+algostring[i]+"PhoIcRecTimeEMMap_i"+to_string(j)); 
                string htitleEM( "CaliV3"+algostring[i]+"IcRecTimeEBMap EM "+to_string(j));
