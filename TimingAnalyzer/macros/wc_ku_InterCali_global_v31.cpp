...
void wc_ku_InterCali_global_v2( string infilename, string outfilename  ){
    ...
    auto MHist = new TH1D( "MHist", "M Histogram", nIterations, 0, nIterations);
    auto MnotHist = new TH1D( "MnotHist", "Mnot Histogram", nIterations, 0, nIterations);
    ...
    auto ebmapkue5 = (TH2D*)fInFile->Get("AveXtalRtStcRecTimeE5EBMap");
    auto epmapkue5 = (TH2D*)fInFile->Get("AveXtalRtStcRecTimeE5EPMap");
    auto emmapkue5 = (TH2D*)fInFile->Get("AveXtalRtStcRecTimeE5EMMap");
    std::cout << " E5 hists: " << ebmapkue5 << " " << epmapkue5 << " " << emmapkue5 << std::endl;
    auto ebmape5 = (TH2D*)fInFile->Get("AveXtalRtOOTStcRecTimeE5EBMap");
    auto epmape5 = (TH2D*)fInFile->Get("AveXtalRtOOTStcRecTimeE5EPMap");
    auto emmape5 = (TH2D*)fInFile->Get("AveXtalRtOOTStcRecTimeE5EMMap");
...
//{sumXtalRtStcPhoIcRecTime, sumXtalRtOOTStcPhoIcRecTime }; //, sumXtalWtOOTStcPhoIcRecTime };
    ...
    std::map<UInt_t,double> sumXtalRtStcPhoIcRecTime;
    std::map<UInt_t,double> sumXtalRtOOTStcPhoIcRecTime;
    std::map<UInt_t,double> sumXtalWtOOTStcPhoIcRecTime;
    std::map<UInt_t,double> numXtalIcRecTime;
    std::map<UInt_t,double> prevDMRtStc;
    std::map<UInt_t,double> prevDMRtOOTStc;
    std::map<UInt_t,double> prevDMWtOOTStc;
    ...
    TH2D * IcMapEB[nAlgos][nIterations+1];
    TH2D * IcMapEP[nAlgos][nIterations+1];
    TH2D * IcMapEM[nAlgos][nIterations+1];
    ...
    for( auto i = 0; i < nAlgos; i++){
		...
		string hnameEB( "AveXtal"+algostring[i]+"PhoIcRecTimeEBMap_i"+to_string(j)); 
                ...
                IcMapEB[i][j] = new TH2D(hnameEB.c_str(),htitleEB.c_str(),171,-85.5,85.5,360,0.5,360.5);
                IcMapEB[i][j]->Sumw2();
                ...
                IcMapEP[i][j] = new TH2D(hnameEP.c_str(),htitleEP.c_str(),100,0.5,100.5,100,0.5,100.5);
                IcMapEP[i][j]->Sumw2();
                ...
                IcMapEM[i][j] = new TH2D(hnameEM.c_str(),htitleEM.c_str(),100,0.5,100.5,100,0.5,100.5);
                IcMapEM[i][j]->Sumw2();
             }
    }
...
//        for (auto ipho = 0; ipho < nPhotons; ipho++){
 ...
 //       } // end loop over photons
    ...
    std::map<UInt_t,double> *  icmaps[nAlgos] = {&sumXtalRtStcPhoIcRecTime, &sumXtalRtOOTStcPhoIcRecTime }; //, sumXtalWtOOTStcPhoIcRecTime };
    std::map<UInt_t,double> *  prevDMmaps[nAlgos] = { &prevDMRtStc, &prevDMRtOOTStc };
    auto previter = iter - 1;  
    if( previter < 0 ) previter = 0;
    for( auto ai = 0; ai < nAlgos; ai++ ){
	 double mddm = 10.0; //(M[iter]/nM[iter]);
         double mfcor = 1.0;
	 std::map<UInt_t,double> curDMmap;
         if( iter == 0 ) mfcor = mfcorrection;
         for( std::map<UInt_t,double>::iterator it=(*icmaps[ai]).begin(); it!=(*icmaps[ai]).end(); ++it){
...
//		   std::cout << "Fill EB hist for Algo " << ai << " at " << fill_idinfo.i2 << " " << fill_idinfo.i1 << " with " << map_time << " for iter " << iter << std::endl;
			   ...
			   //auto gamma = (ci - cim)*((curDMmap[it->first]) - ((*prevDMmaps[ai])[it->first]))/mddm; 
			   //if( gamma == 0 ) gamma = ci*(curDMmap[it->first])/mddm;	
			   auto gamma = 0.0;
			   auto corr = ci - gamma*curDMmap[it->first];
                           if(dothis) std::cout << " " << ci << " " << gamma << " " << curDMmap[it->first] << " " << corr << std::endl;
                           (IcMapEB[ai][iter+1])->Fill( fill_idinfo.i2, fill_idinfo.i1, corr + offset );
...
//                 std::cout << "Fill EM hist for Algo " << ai << " at " << fill_idinfo.i2 << " " << fill_idinfo.i1 << " with " << map_time << " for iter " << iter << std::endl;
	 ...
	 (*prevDMmaps[ai]).clear();
         for( std::map<UInt_t,double>::iterator it=curDMmap.begin(); it!=curDMmap.end(); ++it){
