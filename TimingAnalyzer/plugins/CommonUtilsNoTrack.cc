...
namespace ootNT
{
  ...
  {
    if (ConfigNT::file_exists(inputPaths))
    {
  ...
  {
    if (ConfigNT::file_exists(inputFilters))
    {
  ...
  void PrepNeutralinos(const edm::Handle<std::vector<reco::GenParticle> > & genparticlesH, std::vector<reco::GenParticle> & neutralinos)
  {
    ...
    std::sort(neutralinos.begin(),neutralinos.end(),ootNT::sortByPt);
  }
  ...
  void PrepVPions(const edm::Handle<std::vector<reco::GenParticle> > & genparticlesH, std::vector<reco::GenParticle> & vPions)
  {
    ...
    std::sort(vPions.begin(),vPions.end(),ootNT::sortByPt);
  }
  ...
  void PrepToys(const edm::Handle<std::vector<reco::GenParticle> > & genparticlesH, std::vector<reco::GenParticle> & toys)
  {
    ...
    std::sort(toys.begin(),toys.end(),ootNT::sortByPt);
  }
			  ...
			  const edm::Event & iEvent, std::map<std::string,std::vector<pat::TriggerObjectStandAlone> > & triggerObjectsByFilterMap)
  {
    ...
    for (auto & triggerObjectsByFilterPair : triggerObjectsByFilterMap)
    {
      std::sort(triggerObjectsByFilterPair.second.begin(),triggerObjectsByFilterPair.second.end(),ootNT::sortByPt);
    }
  }
		...
		const float jetEtamax, const int jetIDmin)
  {
    for (const auto & jet : *jetsH)
    {
      ...
      const auto jetID = ootNT::GetPFJetID(jet);
      ...
      std::sort(jets.begin(),jets.end(),ootNT::sortByPt);
    }
  }  
  ...
  void PrepPhotonsCorrectMET(const edm::Handle<std::vector<reco::Photon> > & gedPhotonsH, 
			     const edm::Handle<std::vector<reco::Photon> > & ootPhotonsH,
			     std::vector<reco::Photon> & photons, pat::MET & t1pfMET, const float rho, 
			     const float dRmin, const float phpTmin, const std::string & phIDmin)
  {
    ...
    ootNT::FindOverlapPhotons(gedPhotonsH,nGED,ged_to_oot,ootPhotonsH,nOOT,oot_to_ged,dRmin);

    ///////////////////////////////
    // Produce merged collection //
    ///////////////////////////////

    ootNT::MergePhotons(gedPhotonsH,nGED,ged_to_oot,photons,false,rho,phpTmin,phIDmin);
    ootNT::MergePhotons(ootPhotonsH,nOOT,oot_to_ged,photons,true ,rho,phpTmin,phIDmin);

    ////////////////////////////
    // Sort merged collection //
    ////////////////////////////

    ootNT::SortPhotonsByPt(photons);

    /////////////////////
    // Correct the MET //
    /////////////////////

    ootNT::CorrectMET(gedPhotonsH,nGED,ootPhotonsH,nOOT,oot_to_ged,t1pfMET);
  }

  void FindOverlapPhotons(const edm::Handle<std::vector<reco::Photon> > & gedPhotonsH, const int nGED, std::vector<int> & ged_to_oot,
			  const edm::Handle<std::vector<reco::Photon> > & ootPhotonsH, const int nOOT, std::vector<int> & oot_to_ged,
			  const float dRmin)
  {
	...
	if (ootNT::GetPhotonPt(ootPhoton) > ootNT::GetPhotonPt(gedPhoton)) oot_to_ged[iOOT] = matchedGED; // OOT matches GED, and is greater than GED in pT
  ...
  void MergePhotons(const edm::Handle<std::vector<reco::Photon> > & photonsH, const int nPho, const std::vector<int> & pho_overlap_idxs,
		    std::vector<reco::Photon> & photons, const bool isOOT, const float rho, const float phpTmin, const std::string & phIDmin)
  {
    ...
    for (auto iPho = 0; iPho < nPho; iPho++)
    {
      ...
      // cut on low pt
      if (ootNT::GetPhotonPt(photon) < phpTmin) continue;
      
      // store the GED and OOT VID
      std::vector<reco::Photon::IdPair> idpairs = {{"loose-ged",false}, {"medium-ged",false}, {"tight-ged",false}, {"loose-oot",false}, {"tight-oot",false}};

      ootNT::GetGEDPhoVID(photon,idpairs);      
      if (isOOT) ootNT::GetOOTPhoVID      (photon,idpairs);
      else       ootNT::GetOOTPhoVIDByHand(photon,idpairs,rho);
  ...
  void CorrectMET(const edm::Handle<std::vector<reco::Photon> > & gedPhotonsH, const int nGED,
		  const edm::Handle<std::vector<reco::Photon> > & ootPhotonsH, const int nOOT,
		  const std::vector<int> & oot_to_ged, pat::MET & t1pfMET)
  {
    ...
    for (auto iOOT = 0; iOOT < nOOT; iOOT++)
    {
      ...
      const auto & ootPhoton = (*ootPhotonsH)[iOOT];
      const auto ootet  = ootNT::GetPhotonEt(ootPhoton);
      const auto ootpt  = ootNT::GetPhotonPt(ootPhoton);
	...
	// get gedPhoton info
	const auto & gedPhoton = (*gedPhotonsH)[matchedGED];
	const auto gedet  = ootNT::GetPhotonEt(gedPhoton);
	const auto gedpt  = ootNT::GetPhotonPt(gedPhoton);
    ...
    t1pfMET.setP4(reco::ParticleState::LorentzVector(t1pfMETpx,t1pfMETpy,0,ConfigNT::hypo(t1pfMETpx,t1pfMETpy))); // set new p4
  ...
  void PrunePhotons(std::vector<reco::Photon> & photons,
  ...
  void PruneJets(std::vector<pat::Jet> & jets, const std::vector<reco::Photon> & photons,
		 const int nPhosmax, const float dRmin)
  {
    ...
    for (auto ipho = 0; ipho < nPhos; ipho++)
    {
      ...
      if ( ((eta < ConfigNT::etaEBcutoff) && (HoverE < 0.25) && (Sieie < 0.019)) || ((eta >= ConfigNT::etaEBcutoff && eta < ConfigNT::etaEEmax) && (HoverE < 0.2) && (Sieie < 0.04)) )
      {
  ...
  {
    if      (eta <  ConfigNT::etaEBcutoff)                           return 0.0126*pt+0.000026*pt*pt;
    else if (eta >= ConfigNT::etaEBcutoff && eta < ConfigNT::etaEEmax) return 0.0119*pt+0.000025*pt*pt;
    else                                                           return 0.f;
  }

  float GetGammaPtScale(const float eta, const float pt)
  {
    if      (eta <  ConfigNT::etaEBcutoff)                           return 0.0035*pt;
    else if (eta >= ConfigNT::etaEBcutoff && eta < ConfigNT::etaEEmax) return 0.0040*pt;
    else                                                           return 0.f;
  }
  ...
  void GetGEDPhoVID(const reco::Photon & photon, std::vector<reco::Photon::IdPair> & idpairs)
  {

    // This is where I think the error is coming from

    //std::cout << "Test bool: " << !photon.isPhotonIDAvailable(ConfigNT::GEDPhotonTightVID) << std::endl;

    if ( !photon.isPhotonIDAvailable(ConfigNT::GEDPhotonTightVID) ) return; // My fix

    idpairs[2].second = photon.photonID(ConfigNT::GEDPhotonTightVID);
    idpairs[1].second = photon.photonID(ConfigNT::GEDPhotonMediumVID);
    idpairs[0].second = photon.photonID(ConfigNT::GEDPhotonLooseVID);
  }

  void GetGEDPhoVIDByHand(const reco::Photon & photon, std::vector<reco::Photon::IdPair> & idpairs, const float rho)
  {
    // needed for cuts
    const auto eta = std::abs(photon.superCluster()->eta());
    ...
    const auto ChgHadIso = std::max(photon.chargedHadronIso() - (rho * ootNT::GetChargedHadronEA(eta))                                         ,0.f);
    const auto NeuHadIso = std::max(photon.neutralHadronIso() - (rho * ootNT::GetNeutralHadronEA(eta)) - (ootNT::GetNeutralHadronPtScale(eta,pt)),0.f);
    const auto PhoIso    = std::max(photon.photonIso()        - (rho * ootNT::GetGammaEA        (eta)) - (ootNT::GetGammaPtScale        (eta,pt)),0.f);
    
    if (eta < ConfigNT::etaEBcutoff)
    {
    ...
    else if (eta >= ConfigNT::etaEBcutoff && eta < ConfigNT::etaEEmax)
    {
  ...
  void GetOOTPhoVID(const reco::Photon & photon, std::vector<reco::Photon::IdPair> & idpairs)
  {

    if ( !photon.isPhotonIDAvailable(ConfigNT::OOTPhotonTightVID) ) return; // My fix

    idpairs[4].second = photon.photonID(ConfigNT::OOTPhotonTightVID);
    idpairs[3].second = photon.photonID(ConfigNT::OOTPhotonLooseVID);
  }

  void GetOOTPhoVIDByHand(const reco::Photon & photon, std::vector<reco::Photon::IdPair> & idpairs, const float rho)
  {
    // needed for cuts
    const auto eta = std::abs(photon.superCluster()->eta());
    const auto pt  = photon.pt();
    ...
    const auto HoverE = photon.hadTowOverEm();
    const auto Sieie  = photon.full5x5_sigmaIetaIeta();
    const auto EcalPFClIso = std::max(photon.ecalPFClusterIso() - (rho * ootNT::GetEcalPFClEA(eta)) - (ootNT::GetEcalPFClPtScale(eta,pt)),0.f);
    const auto HcalPFClIso = std::max(photon.hcalPFClusterIso() - (rho * ootNT::GetHcalPFClEA(eta)) - (ootNT::GetHcalPFClPtScale(eta,pt)),0.f);
    const auto TrkIso      = std::max(photon.trackIso()         - (rho * ootNT::GetTrackEA   (eta)) - (ootNT::GetTrackPtScale   (eta,pt)),0.f);
  ...
  void SplitPhotons(std::vector<reco::Photon> & photons, const int nmax)
  {
    ...
    std::vector<reco::Photon> tmpphotons;
  ...
  void StoreOnlyPho(std::vector<reco::Photon> & photons, const int nmax, const bool isOOT)
  {
    std::vector<reco::Photon> tmpphotons;
  ...
  void DumpPhoton(const reco::Photon & photon, const bool isOOT,
		  const EcalRecHitCollection * recHitsEB,
		  const EcalRecHitCollection * recHitsEE)
  {
    std::cout << (isOOT ? "OOT" : "GED") << " Photon Info -->"
	      << " pT: " << std::setprecision(3) << ootNT::GetPhotonPt(photon) 
